pipeline {
    ...
    stages {
       ...
       stage('DelayPostmanTest') {
        ...
        stage('Postman') {
            steps {
              sh 'newman run PostmanReviewFiles/Spring_PetClinic_Copy.postman_collection.json -e PostmanReviewFiles/PetClinic_Environment.postman_environment.json -- reporters junit'
            }
                post {
                always {
                    junit '**/TEST*.xml'
